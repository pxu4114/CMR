...
class Seq2SeqAutoencoder(nn.Module):
	...
	def __init__(self, num_features, noise_prob=0.3):
		...
		self.encoder = Encoder(num_features=num_features, num_units=50, num_layers=2)
		self.decoder = Decoder(num_features=num_features, num_units=50, num_layers=2, output_dim=num_features)
	...
	def train_step(self, input_dat, lengths):
		...
		_, encoder_hidden_state = self.encoder.forward(noised_input.float(), lengths, hidden=encoder_hidden_state)
	...
	def train(self, dataloader, dev_dataloader, epochs, epochs_per_save, model_save_path):
		device = torch.device(DEVICE_ID if torch.cuda.is_available() else "cpu")
		...
		for epoch in range(epochs):
			...
			if len(eval_losses) > 2 and eval_losses[len(eval_losses)-2] - eval_losses[len(eval_losses)-1] < 0.1:
				print("reducing lr!")
...
def main():
	...
	dev_dataset = AudioMFCCDataset(aud_mfcc_file=os.path.join(data_path, "dev_aud_mfcc_std_trimmed.npy"), 
								aud_mfcc_lengths=os.path.join(data_path,"dev_sample_lengths_std_trimmed.npy"))
	...
	losses = gpu_aud2vec.train(loader, dev_loader, epochs, epochs_per_save, args.model_save_path)
